...
export const Chat: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    console.log('Send button clicked', {
      ...
      currentConversationId,
      hasInput: !!input.trim(),
                canSend: !isLoading && !!input.trim() && !!currentConversationId
    });
    ...
    if (!input.trim() || isLoading || !currentConversationId) {
      console.log('Send blocked:', {
        noInput: !input.trim(),
                  loading: isLoading,
                  noConversation: !currentConversationId
      });
      return;
    }
    ...
    try {
      setIsLoading(true);
      setError(null);
      console.log('Sending message:', { content: input.trim(), conversationId: currentConversationId });
  ...
  return (
    ...
    Press Enter to send • {Array.isArray(messages) ? messages.length : 0} message{Array.isArray(messages) && messages.length !== 1 ? 's' : ''} 💫
    </p>
